...
fn main() {
    ...
    // simulate
    for _ in 0..6 {
        let min = field.keys().fold((0, 0, 0, 0), |m, v| {
            (
                cmp::min(m.0, v.0 - 1),
                cmp::min(m.1, v.1 - 1),
                cmp::min(m.2, v.2 - 1),
        ...
        let max = field.keys().fold((0, 0, 0, 0), |m, v| {
            (
                cmp::max(m.0, v.0 + 1),
                cmp::max(m.1, v.1 + 1),
                cmp::max(m.2, v.2 + 1),
            ...
            for z in min.2..=max.2 {
                for y in min.1..=max.1 {
                    for x in min.0..=max.0 {
                        let value = field.contains_key(&(x, y, z, w));
                        let mut active = 0;
                        for dx in -1..=1 {
                            for dy in -1..=1 {
                                for dz in -1..=1 {
                                    for dw in -1..=1 {
                                        ...
                                        if field.contains_key(&(x + dx, y + dy, z + dz, w + dw)) {
                                            active += 1;
                                        }
                                    }
                                }
                        ...
                        if nextvalue {
                            next.insert((x, y, z, w).to_owned(), nextvalue);
                        }
                    }
                }
            }
