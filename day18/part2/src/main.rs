...
fn plus_step(s:String) -> String
{
    ...
    let replace = calc(c[0].to_owned());
    return s.replacen(&search,&replace,1);
}

fn plus_simplify(s:String) -> String
{
    let mut expression = s;
    let mut new_expression:String;
    loop {
        new_expression = plus_step(expression.to_string());
...
fn step(s:String) -> String
{
    ...
    let c = captures.unwrap();
    let search = c[0].to_owned();
    let evaled = calc(plus_simplify(c[2].to_owned()));
    let replace = format!("{}{}{}",c[1].to_owned(),evaled,c[3].to_owned());
    return s.replacen(&search,&replace,1);
}
